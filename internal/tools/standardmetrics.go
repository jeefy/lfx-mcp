...
An LF-wide question takes NO project; the tlf slug is one bucket, not the LF-wide scope.`
...
type StandardMetricsArgs struct {
	...
	Org          string `json:"org,omitempty" jsonschema:"Optional organization scope: ONE stored legal account name from search_b2b_orgs, exact (e.g. Red Hat LLC). A name matching no data-bearing account is rejected with candidates; never guess one. Families whose model carries no account (the meetup families among them) reject org."`
