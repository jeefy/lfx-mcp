...
func TestStandardMetricsGuidanceContent(t *testing.T) {
	...
	for _, want := range []string{
		...
		"the average is the v2 health score normalized to a hundred-point scale (raw v2 points over each project's coverage-dependent maximum) from the DBT-1 deployment (week of 2026-09-07) onward, v1 before",
		"except by=population,\n  which reads the v2 average already",
		"except by=population, which reads the v2 average already. v2 score;",
		"normalized to a hundred-point scale (raw v2 points over each project's\n  coverage-dependent maximum)",
		"The family reports the count of\n  scored projects and their mean score",
		"before that day the layer's average still reads\n  the v1 score",
		...
		"Until the lf-dbt DBT-1 deployment lands, call the average the v1 score; the count and the categories are v2 — except by=population",
		"Until the lf-dbt DBT-1 deployment lands, call the average the\n  v1 score",
		"sibling fund\nproject",
