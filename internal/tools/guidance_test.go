...
func TestStandardMetricsGuidanceContent(t *testing.T) {
	...
	for _, want := range []string{
		...
		"that is member_organizations (paying-only: paying_member_organizations), never derived from these rows",
		...
		"ORGANIZATIONS, not memberships",
		"Firstness follows the scope and applied.firstness names it",
		"first membership in ANY of the consortium's projects",
		"a project below foundation level with combined is REJECTED",
		"The organization grain of churn",
		...
		"`consortium_members` lists every slug merged",
		"subprojects=excluded reads the named project alone and merges\nnothing",
		"first-membership is defined for a single project\n  or for a whole foundation",
		"member_organizations\n  (paying-only: paying_member_organizations; new to the LF:\n  new_member_organizations; lost: lost_member_organizations)",
		...
		"placeholder accounts ('Individual - No Account', 'TI Account') are folded into the NULL account row on by=org",
		"report that row as unattributed learners, never as an organization",
