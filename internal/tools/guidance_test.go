...
func TestSemanticLayerGuidanceContent(t *testing.T) {
	...
	for _, want := range []string{
		...
		"this layer defaults to the trailing 365 days, which cannot\ngive whole calendar years",
		"the upper bound is\nthe day after today (exclusive) so the last row is year-to-date",
		"(the meetup families read all history\ninstead — set start_date for any comparison)",
...
func TestStandardMetricsGuidanceContent(t *testing.T) {
	...
	for _, want := range []string{
		...
		"absent rather than a zero row; NULL region or city rows are chapters with none set",
