...
func TestSemanticLayerGuidanceContent(t *testing.T) {
	...
	for _, want := range []string{
		...
		"SPEAKERS BY COMPANY",
		"report it as a floor",
		"as PEOPLE, not talks",
		"TALKS BY TOPIC",
		"is the talks standard metric",
		"counts accepted SESSIONS (speakers counts people)",
		"talks mentioning\nOpenTelemetry",
		"This layer has no session entity",
		"talks is\nthe one family with a filter of its own, topic",
		"the foundation that ran the\nevent, not what a talk was about",
...
func TestStandardMetricsGuidanceContent(t *testing.T) {
	...
	for _, want := range []string{
		...
		"how many Red Hat employees gave talks\n  at KubeCon",
		"The figure is people, not talks",
