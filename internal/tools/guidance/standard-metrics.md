...
| new_member_organizations | window | total, project, foundation, country, region | Distinct organizations whose first membership was installed in the window; applied.firstness says first ever, first in the foundation or first in the project, following the project switch | [project, project_name / foundation / country / region,] new_member_organizations | "New logos" when no project is named; a lapsed account that rejoins is not new (contrast new_members); with an org named, total reads zero or one per window — the answer to "when did X first join"; all history unless start_date |
| lost_member_organizations | window | total, project, foundation, country, region | Distinct organizations whose LAST membership lapsed in the window, counted at that term's churn date (the day after it ended), and that hold no active membership as of the build | [project, project_name / foundation / country / region,] lost_member_organizations | An organization that dropped one project but kept another is churned on membership_churn and NOT lost here; by=project and by=foundation are where the last membership sat; all history unless start_date |
...
it): first LF membership ever with no project, first in the foundation (or
