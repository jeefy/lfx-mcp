...
| maintainers | at-date | total, org, project, maintainer, role, source | Active maintainers today (LF projects only); with period, today's roster active in each period | [account / foundation, project, project_name / project, project_name, handle, maintainer, account, role, source / role / source,] active_maintainers[, active_maintainers_excl_reviewers, active_reviewers, active_maintainers_excl_inherited] | Distinct people; LF projects only: an ad hoc count over the whole maintainers index reads higher; the NULL account row is maintainers with no resolved employer; today only unless period; by=maintainer has no series; per-project counts include people the roster inherits from vendored Linux kernel trees on kernel-fork projects, and MAINTAINERS-file reviewers count as maintainers: by=total, org and project carry the three split columns beside active_maintainers (excl_reviewers and reviewers partition it within one project; excl_inherited drops people whose roster came only with a vendored kernel tree or another seeded roster), by=role (maintainer, reviewer) and by=source (project_repo, inherited_kernel_tree, roster_repo) give one row per value; a person maintaining two projects counts once in each and can sit in two role or source rows; by=maintainer carries handle, a GitHub profile URL of the shape the contributions families return under that word (NULL where the row's identity is not a GitHub login); match the two on the login part |
...
| talks | window | total, event, track, topic, format, org, speaker | Accepted sessions (talks, keynotes, lightning talks, workshops) of events starting in the window | [event / track / topic / format / account, parent_org / speaker, account,] talks | Counts SESSIONS, not people: a session with three speakers is one talk, a person with two sessions is two; speakers is the people reading. Accepted status only, like speakers. topic is a case-insensitive keyword match on title, track and abstract — say "talks mentioning X", not "talks about X"; by=topic groups on the event's own track/tag vocabulary, which differs per event. by=org attributes a talk to every distinct speaker account on it, so org rows can exceed the total; a large share of speakers resolve to no account and sit in the NULL row (a floor). by=speaker is one row per person per event with their talk count |
...
- contributions by=contributor and maintainer_contributions by=maintainer
  ...
  roster: one row per person, project, employer and role, with `handle` the
  GitHub profile URL — the shape the contributions readings return under
  the same word; match the two on
  the login part (after the last slash), case-insensitively, as the stored
  contributions URL is not one uniform prefix; NULL where the identity is
  not a GitHub login. A roster can list people a project's published
  governance file marks emeritus: the source records the role, and tenure
  ends only when the person leaves the file, so match on handle and let the
  published file settle status.
...
- A roster against a published list: maintainers, project=<slug>,
...
- Talks on a topic at an event ("how many OpenTelemetry talks occurred at
  ...
  sessions whose title, track or abstract mentions OpenTelemetry" — a
